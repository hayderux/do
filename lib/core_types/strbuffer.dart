...
Future<ASTNode> visitStrBufferProperties(
    ASTNode node, ASTNode left) async {
...
ASTNode visitStrBufferMethods(ASTNode node, ASTNode left) {
  switch (node.binaryOpRight.funcCallExpression.variableName) {
    ...
    case 'write':
      ...
      return left
        ..strBuffer
            .write((node.binaryOpRight.functionCallArgs[0]).stringValue);

    case 'writeAll':
      expectArgs(node.binaryOpRight.functionCallArgs, [ListNode]);

      for (var e in node.binaryOpRight.functionCallArgs[0].listElements) {
    ...
    case 'writeAsciiCode':
      ...
      return left
        ..strBuffer.writeCharCode(
            (node.binaryOpRight.functionCallArgs[0]).intVal);
