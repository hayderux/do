...
class Runtime {
  ...
  void collectAndSweepGarbage(List oldDefList, Scope scope) {
    ...
    scope.variableDefinitions
        .where((newDef) => !oldDefList.contains(newDef))
  ...
  Future<ASTNode> runtimeFunctionCall(ASTNode fCall, ASTNode fDef) async {
    if (fCall.functionCallArgs.length != fDef.functionDefArgs.length ||
        fCall.namedFunctionCallArgs.length >
            fDef.namedFunctionDefArgs.length) {
    ...
    for (int i = 0; i < fCall.functionCallArgs.length; i++) {
      ...
      if (astArg.type == ASTType.AST_VARIABLE) {
        final vDef =
            await getVarDefByName(getScope(astArg), astArg.variableName);
      ...
      final DATATYPE varDefValType =
          newVariableDef.variableType.typeValue.type;

      if (varDefValType != DATATYPE.DATA_TYPE_VAR &&
          !varDefValType.toString().endsWith(newVariableDef
              .variableValue.type
              .toString()
              .replaceFirst('ASTType.AST_', ''))) {
        throw UnexpectedTypeException(
            '[Line ${fCall.lineNum}] Invalid type');
      }

      funcDefBodyScope.variableDefinitions.addFirst(newVariableDef);
    }

    for (var e in fDef.namedFunctionDefArgs) {
      funcDefBodyScope.variableDefinitions
          .addFirst(e..variableValue = NoSeebNode());
    }

    for (int i = 0; i < fCall.namedFunctionCallArgs.length; i++) {
      final ASTNode astArg = fCall.namedFunctionCallArgs[i];
      final ASTNode astFDefArg = fDef.namedFunctionDefArgs.firstWhere(
          (e) => e.variableName == astArg.variableName, orElse: () {
        throw InvalidArgumentsException(
            'No defined parameter `${astArg.variableName}` for `${fDef.funcName}` was found');
      });

      final String argName = astFDefArg.variableName;

      final VarDefNode newVariableDef = VarDefNode()
        ..variableType = astFDefArg.variableType;

      if (astArg.variableValue.type == ASTType.AST_VARIABLE) {
        final vDef = await getVarDefByName(getScope(astArg.variableValue),
            astArg.variableValue.variableName);
  ...
  ASTNode registerGlobalFunction(
      String fName, AstFuncPointer funcPointer) {
  ...
  ASTNode registerLocalFunction(
      String fName, AstFuncPointer funcPointer, Scope scope) {
    final fDef = FuncDefNode()
      ..funcName = fName
  ...
  ASTNode registerGlobalFutureFunction(
      String fName, AstFutureFuncPointer funcPointer) {
    final fDef = FuncDefNode()
      ..funcName = fName
    ...
    switch (node.type) {
      ...
      case ASTType.AST_THROW:
        ...
        throwArg.classChildren.whereType<VarDefNode>().forEach((varDef) {
          classToString +=
              '\t${varDef.variableName}: ${varDef.variableValue.toString()}\n';
        });
      ...
      case ASTType.AST_ANY:
        return node;
      default:
        throw UncaughtStatementException(
            'Uncaught statement ${node.type}');
    }
  }
  ...
  Future<ASTNode> visitVariable(ASTNode node) async {
      ...
      for (int i = 0; i < node.enumElements.length; i++) {
        ...
        if (variable.variableName == node.variableName) {
          if (variable.ast != null) {
            ...
            final listAST = ListNode()
              ..listElements = [i, variable.variableName];
    ...
    if (localScope != null) {
      ...
      for (int i = 0; i < localScope.functionDefinitions.length; i++) {
        final ASTNode funcDef =
            localScope.functionDefinitions.elementAt(i);
    ...
    if (!node.isClassChild && globalScope != null) {
      ...
      for (int i = 0; i < globalScope.functionDefinitions.length; i++) {
        final ASTNode funcDef =
            globalScope.functionDefinitions.elementAt(i);
    ...
    stack.reversed.take(5).forEach((Map item) => stacktrace +=
        ' [Line:${item['line']}] $path::${item['function']}\n');
      ...
      for (int i = 0; i < node.classChildren.length; i++) {
        ...
        if (objectVarDef.variableName == left.variableName) {
          ...
          if (!objectVarDef.isNullable && node.variableValue == null ||
              node.variableValue is NoSeebNode) {
            String stacktrace =
                '\nThe stacktrace when the error was thrown was:\n';
          ...
          case TokenType.TOKEN_PLUS_PLUS:
            {
              final ASTNode variable =
                  await visitVariable(node.binaryOpRight);
          ...
          case TokenType.TOKEN_PLUS_PLUS:
            {
              ...
              if (variable.typeValue.type == DATATYPE.DATA_TYPE_INT) {
                ...
                variable.intVal++;
                return newVariable;
              } else if (variable.typeValue.type ==
  ...
  Future<ASTNode> runtimeFuncLookup(Scope scope, ASTNode node) async {
    ...
    if (funcDef.futureFuncPointer != null) {
      ...
      final ret = await visit(await funcDef.futureFuncPointer(
          this, funcDef, visitedFuncPointerArgs));
    ...
    if (funcDef.funcPointer != null) {
      ...
      final ret = await visit(
          funcDef.funcPointer(this, funcDef, visitedFuncPointerArgs));
  ...
  Future<ASTNode> visitFuncCall(ASTNode node) async {
    stack.add({
      'line': node.lineNum,
      'function': node.funcCallExpression.variableName
    });
  ...
  Future<ASTNode> visitAttAccess(ASTNode node) async {
    ...
    if (node.binaryOpRight != null &&
      ...
      switch (left.type) {
        ...
        case ASTType.AST_CLASS:
          ...
          if (binOpRight.type == ASTType.AST_VARIABLE ||
            ...
            final ASTNode classPropertyNode =
                visitClassProperties(node, left);
          ...
          break;
        case ASTType.AST_LIST_ACCESS:
          node.binaryOpRight.binaryOpLeft.classChildren =
              left.classChildren;
      ...
      if (node.binaryOpRight.type == ASTType.AST_FUNC_CALL) {
        if (node.binaryOpRight.funcCallExpression.type ==
            ASTType.AST_VARIABLE) {
          final funcCallName =
              node.binaryOpRight.funcCallExpression.variableName;
            ...
            for (int i = 0; i < left.funcDefinitions.length; i++) {
              ...
              if (fDef.funcName == funcCallName) {
                if (fDef.funcPointer != null) {
                  ...
                  for (int j = 0;
                      j < node.binaryOpRight.functionCallArgs.length;
                      j++) {
                    final ASTNode astArg =
                        node.binaryOpRight.functionCallArgs[j];
                    final visited = await visit(astArg);
                    visitedFuncPointerArgs.add(visited);
                  }

                  return await visit(fDef.funcPointer(
                      this, left, visitedFuncPointerArgs));
                }
              }
            }
          }
                ...
                if (objChild.funcName == funcCallName) {
                  if (objChild.funcPointer != null) {
                    ...
                    for (int j = 0;
                      ...
                      final ASTNode astArg =
                          node.binaryOpRight.functionCallArgs[j];
                      final ASTNode visited = await visit(astArg);
                      visitedFuncPointerArgs.add(visited);
                    }

                    return await visit(objChild.funcPointer(
                        this, left, visitedFuncPointerArgs));
                  }

                  return await runtimeFunctionCall(
                      node.binaryOpRight, objChild);
                }
              }
            }
          }
    ...
    if (ast.type == ASTType.AST_STRING) {
      ...
      if (left.map.containsKey(key)) {
        if (left.map[key] is String) {
          final ASTNode mapValAST = StringNode()
  ...
  Future<ASTNode> visitBinaryOp(ASTNode node) async {
          ...
          if (child.type == ASTType.AST_VARIABLE_DEFINITION) {
            ...
            if (child.funcName == accessName) {
              for (int j = 0; j < right.functionCallArgs.length; j++) {
                ...
                getScope(child.functionDefBody)
                    .variableDefinitions
                    .addFirst(newVarDef);
              }
    ...
    switch (node.binaryOperator.type) {
      case TokenType.TOKEN_PLUS:
        {
          if (left.type == ASTType.AST_INT &&
              right.type == ASTType.AST_INT) {
            retVal = IntNode();
          ...
          if (left.type == ASTType.AST_STRING &&
            ...
            retVal.stringValue =
                left.stringValue + right.intVal.toString();
          ...
          if (left.type == ASTType.AST_INT &&
            ...
            retVal.stringValue =
                left.intVal.toString() + right.stringValue;
          ...
          if (left.type == ASTType.AST_STRING &&
            ...
            retVal.stringValue =
                left.stringValue + right.doubleVal.toString();
          ...
          if (left.type == ASTType.AST_DOUBLE &&
            ...
            retVal.stringValue =
                left.doubleVal.toString() + right.stringValue;
      ...
      case TokenType.TOKEN_EQUALITY:
        {
          ...
          if (left is ClassNode && right is ClassNode) {
            retVal = BoolNode()
              ..boolVal = left.className == right.className;
      ...
      case TokenType.TOKEN_NOT:
        {
          ...
          switch (node.unaryOpRight.type) {
            case ASTType.AST_VARIABLE:
              {
                boolAST.boolVal =
                    !boolEval(await visitVariable(node.unaryOpRight));
                return boolAST;
              }
              break;
            default:
  ...
  Future<ASTNode> visitSwitch(ASTNode node) async {
    ...
    switch (caseAST.type) {
      case ASTType.AST_STRING:
        final Iterable<ASTNode> testCase = node.switchCases.keys.where(
            (element) => element.stringValue == caseAST.stringValue);
      ...
      case ASTType.AST_MAP:
        final Iterable<ASTNode> testCase = node.switchCases.keys
            .where((element) => element.map == caseAST.map);
      ...
      case ASTType.AST_LIST:
        final Iterable<ASTNode> testCase = node.switchCases.keys.where(
            (element) => element.listElements == caseAST.listElements);
  ...
  Future<ASTNode> visitNew(ASTNode node) async =>
      (await visit(node.newValue)).copy();
  ...
  Future<ASTNode> visitIterate(ASTNode node) async {
    ...
    for (int j = fDefBodyScope.variableDefinitions.length - 1;
        j > 0;
        j--) {
      fDefBodyScope.variableDefinitions
          .remove(fDefBodyScope.variableDefinitions.elementAt(i));
    }
    ...
    if (astIterable.type == ASTType.AST_STRING) {
      ...
      for (; i < astIterable.listElements.length; i++) {
        newVarDef.variableValue =
            await visit(astIterable.listElements[i] as ASTNode);
  ...
  Future<ASTNode> visitAssert(ASTNode node) async {
    ...
    if (!boolEval(boolAST)) {
      ...
      if (node.assertExpression.type == ASTType.AST_BINARYOP) {
        final left =
            (await visit(node.assertExpression.binaryOpLeft)).toString();
        final right =
            (await visit(node.assertExpression.binaryOpRight)).toString();
...
void expectArgs(List inArgs, List<Type> args) {
  ...
  for (int i = 0; i < args.length; i++) {
    ...
    if (ast.runtimeType != args[i]) {
      print(
          'Received argument of type ${ast.runtimeType}, but expected ${args[i]}');
