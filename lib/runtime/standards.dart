...
Future<void> initStandards(Runtime runtime, String path) async {
  ...
  runtime.registerGlobalVariable(
      'birbVer', StringNode()..stringValue = '0.0.1');
  ...
  // Functions
  runtime.registerGlobalFunction('print', funcScrem);
  runtime.registerGlobalFunction('printLn', funcScremLn);
  runtime.registerGlobalFunction('printF', funcScremF);
  ...
  runtime.registerGlobalFutureFunction(
      'variableFromString', funcVarFromString);
}

Future<ASTNode> funcGrab(
  ...
  if (astStr.stringValue.startsWith('birb:')) {
    ...
    final Lexer lexer = Lexer(File(
            '${Directory.current.path}${ps}core$ps$fileName$ps$fileName.birb')
        .readAsStringSync());
    ...
    switch (fileName) {
      ...
      default:
        throw UnexpectedTokenException(
            'Error [Line ${self.lineNum}]:Dart file $fileName no found.');
    }
  ...
  final String filename =
      '$filePath${Platform.pathSeparator}${astStr.stringValue}';
  final ASTNode node =
      Parser(Lexer(File(filename).readAsStringSync())).parse();
  await runtime.visit(node);
...
ASTNode funcScrem(Runtime runtime, ASTNode self, List<ASTNode> args) {
  for (int i = 0; i < args.length; i++) {
    ...
    if (astArg is BinaryOpNode) {
      Runtime(filePath)
          .visitBinaryOp(astArg)
          .then((value) => astArg = value);
    }
    ...
    if (str == null) {
      throw const UnexpectedTokenException(
          'Screm must contain non-null arguments');
    }
...
ASTNode funcScremF(Runtime runtime, ASTNode self, List<ASTNode> args) {
  if (args[0] is! StringNode) {
    throw UnexpectedTypeException(
        'Error [Line ${self.lineNum}]: scremF expects the first parameter to be a String');
  }
  ...
  if (str == null) {
    throw UnexpectedTokenException(
        'Error [Line ${self.lineNum}]: Screm must contain non-null arguments');
  }
...
ASTNode dateClass(Runtime runtime) {
  ...
  astVarWeekDay.variableType.typeValue =
...
ASTNode timeClass(Runtime runtime) {
  ...
  astVarMinute.variableType.typeValue =
      DataType.as(DATATYPE.DATA_TYPE_INT);
  ...
  astVarSeconds.variableType.typeValue =
      DataType.as(DATATYPE.DATA_TYPE_INT);
...
ASTNode listClass(Runtime runtime) {
  ...
  astObj.classChildren.add(FuncDefNode()
    ..funcName = 'filled'
...
Future<ASTNode> funcVarFromString(
    Runtime runtime, ASTNode self, List<ASTNode> args) async {
  expectArgs(args, [StringNode]);

  return await runtime.getVarDefByName(
      runtime.getScope(args[0]), args[0].stringValue);
}
