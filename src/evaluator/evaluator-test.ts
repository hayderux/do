...
`,
    ...
    {
      input: "foobar",
      expected: "identifier not found: foobar at line 1, column 1",
    },
    {
      input: '"Hello" - "World"',
...
export function TestVarStatements(t: Test) {
  let tests = [
...
function TestArrayLiterals(t: Test) {
  ...
  if (result.Elements.length !== 3) {
    t.Fatalf(
      "array has wrong num of elements. got=%s, want=3",
...
function testIntegerObject(t: Test, obj: OObject, expected: number): boolean {
  ...
  if (!(result instanceof OInteger)) {
    t.Errorf(
      "object is not OInteger. got=%s(%s)",
...
function testBooleanObject(t: Test, obj: OObject, expected: boolean): boolean {
  ...
  if (!(result instanceof OBoolean)) {
    t.Errorf(
      "object is not OBoolean. got=%s(%s)",
      result.constructor.name,
      Object.values(result)
    );
    return false;
