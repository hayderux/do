...
export default function Eval(
  ...
  // NullLiteral
    ...
    if (node.Index) return evalVarStatement(node, env);
    // simple value set
...
function evalSwitchExpression(
  ...
  let condition = Eval(se.Value, env);
  if (isError(condition)) return condition;
  ...
  for (let i = 0; i < cases.length; i++) {
    ...
    if (c.Value === null) {
      ...
      let condition = Eval(c.Value, env);
      if (isError(condition)) return condition;
      if (isTruthy(condition)) {
...
function evalHashLiteral(node: HashLiteral, env: Environment): OObject {
  ...
  node.Pairs.forEach((valueNode, keyNode) => {
    ...
    if (
      !(
        hashKey instanceof OBoolean ||
        hashKey instanceof OInteger ||
        hashKey instanceof OString
      )
    ) {
...
function evalHashIndexExpression(hash: OObject, index: OObject): OObject {
  ...
  if (!(hashObject instanceof OHash)) {
    return newError(
      "unusable as hash object: %s",
      (hashObject as OObject).Type()
    );
  }
  ...
  if (
    !(
      key instanceof OBoolean ||
      key instanceof OInteger ||
      key instanceof OString
    )
  ) {
